...
| #V | Hash git | Qué hizo |
|----|----------|----------|
| **#V22** | `HASH` | **Panel: indicador de versión + aviso de clave** — píldora `#appVersion` (v22) en el brand para saber al instante qué versión se está viendo, banner ámbar `#noKeyHint` cuando falta la `MOSTRADOR_API_KEY` (en vez de paneles vacíos), cache-busting `?v=22`. Verificado con navegador real: el cambio de vistas es correcto y todos los endpoints responden 200. Detalle: [[Indicador_Version_y_Clave_2026-09-22]]. |
